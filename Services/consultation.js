...
module.exports = {
  async createConsultationReq(dataToConsultation, dataToConsultationRequests) {
    let check_sql = await mysql.query(
      `select * from undergraduates where email='${dataToConsultationRequests.undergraduate_email}'`
    );
    if (check_sql[0] == null) {
      return;
    }
  ...
  async createConsultationReqProvide(dataToConsultation, dataToRequestprovide) {
    let check_sql = await mysql.query(
      `select * from undergraduates where email='${dataToConsultation.undergraduate_email}'`
    );
  ...
  async getPendingRequests() {
    let sql =
      "SELECT u.undergradFName,u.undergradLName, u.batch, u.faculty,u.email,co.requestStatus,co.sessionType,c.consultantFName,c.consultantLName,cp.consultation_id FROM consultation co INNER JOIN consultationRequests cr ON co.requestStatus=0 AND co.consultationId=cr.consultation_id INNER JOIN consultationRequestprovide cp ON co.consultationId=cp.consultation_id INNER JOIN undergraduates u ON cr.undergraduate_email=u.email INNER JOIN consultants c ON cp.consultantId=c.consultantId ORDER BY reqTimeStamp desc";
    let result = await mysql.query(sql);
  ...
  async getIndividualSessions() {
    let sql =
      'SELECT u.undergradFName,u.undergradLName, u.batch, u.faculty,u.email,co.requestStatus,co.sessionType,c.consultantFName,c.consultantLName,cp.consultation_id FROM consultation co INNER JOIN consultationRequests cr ON co.requestStatus=0 AND co.sessionType="Individual Career Counseling Sessions" AND co.consultationId=cr.consultation_id INNER JOIN consultationRequestprovide cp ON co.consultationId=cp.consultation_id INNER JOIN undergraduates u ON cr.undergraduate_email=u.email INNER JOIN consultants c ON cp.consultantId=c.consultantId ORDER BY reqTimeStamp desc';
    let result = await mysql.query(sql);
    if (result) {
      return result;
    }
  ...
  async getRoundTableDiscussions() {
    let sql =
      'SELECT u.undergradFName,u.undergradLName, u.batch, u.faculty,u.email,co.requestStatus,co.sessionType,c.consultantFName,c.consultantLName,cp.consultation_id FROM consultation co INNER JOIN consultationRequests cr ON co.requestStatus=0 AND co.sessionType="Round Table Discussion" AND co.consultationId=cr.consultation_id INNER JOIN consultationRequestprovide cp ON co.consultationId=cp.consultation_id INNER JOIN undergraduates u ON cr.undergraduate_email=u.email INNER JOIN consultants c ON cp.consultantId=c.consultantId ORDER BY reqTimeStamp desc';
    let result = await mysql.query(sql);
    if (result) {
      return result;
    }
  ...
  async getSoftSkillDevelopment() {
    let sql =
      'SELECT u.undergradFName,u.undergradLName, u.batch, u.faculty,u.email,co.requestStatus,co.sessionType,c.consultantFName,c.consultantLName,cp.consultation_id FROM consultation co INNER JOIN consultationRequests cr ON co.requestStatus=0 AND co.sessionType="Soft Skill Development" AND co.consultationId=cr.consultation_id INNER JOIN consultationRequestprovide cp ON co.consultationId=cp.consultation_id INNER JOIN undergraduates u ON cr.undergraduate_email=u.email INNER JOIN consultants c ON cp.consultantId=c.consultantId ORDER BY reqTimeStamp desc';
    let result = await mysql.query(sql);
    if (result) {
      return result;
    }
  ...
  async getForeignExpertSessions() {
    let sql =
      'SELECT u.undergradFName,u.undergradLName, u.batch, u.faculty,u.email,co.requestStatus,co.sessionType,c.consultantFName,c.consultantLName,cp.consultation_id FROM consultation co INNER JOIN consultationRequests cr ON co.requestStatus=0 AND co.sessionType="Session with an Expert in a Foreign University" AND co.consultationId=cr.consultation_id INNER JOIN consultationRequestprovide cp ON co.consultationId=cp.consultation_id INNER JOIN undergraduates u ON cr.undergraduate_email=u.email INNER JOIN consultants c ON cp.consultantId=c.consultantId ORDER BY reqTimeStamp desc';
    let result = await mysql.query(sql);
    if (result) {
      return result;
    }
  ...
  async getMockupInterviews() {
    let sql =
      'SELECT u.undergradFName,u.undergradLName, u.batch, u.faculty,u.email,co.requestStatus,co.sessionType,c.consultantFName,c.consultantLName,cp.consultation_id FROM consultation co INNER JOIN consultationRequests cr ON co.requestStatus=0 AND co.sessionType="Mockup Interviews" AND co.consultationId=cr.consultation_id INNER JOIN consultationRequestprovide cp ON co.consultationId=cp.consultation_id INNER JOIN undergraduates u ON cr.undergraduate_email=u.email INNER JOIN consultants c ON cp.consultantId=c.consultantId ORDER BY reqTimeStamp desc';
    let result = await mysql.query(sql);
    if (result) {
      return result;
    }
