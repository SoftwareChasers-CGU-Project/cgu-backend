...
app.get("/comSessions/pending", verifyToken, async (req, res) => {
  ...
  app.get("/comSessions/past", async (req, res) => {
    try {
      ...
      if (allPast) {
        return res.status(200).send(allPast);
      }
    } catch (error) {
      console.log(error, "error!!");
    }
...
app.delete("/comSessions/:sessionId", async (req, res) => {
  try {
    ...
    let info = await transporter.sendMail({
      ...
      text: reason,
      html:
        "<p>Dr madam/sir,</p><p>Your session request to conduct a session on " +
        email[0].sessionTopic +
        " was rejected by the Career Guidance Unit of university of Moratuwa.<p><b>The reason for rejecting session:</b></p>" +
        reason +
  ...
  app.post("/comSessions", async (req, res) => {
    try {
      ...
      if (createSession) {
        return res.status(200).send(createSession);
      }
    ...
    try {
      ...
      let info = await transporter.sendMail({
        ...
        html:
          "<p>Dr madam/sir,</p><p>Your session request to conduct a session on " +
          email[0].sessionTopic +
          " was accepetd by the Career Guidance Unit of university of Moratuwa.</p><p>Thank you</p>Career Guidance Unit,<br><p>University of Moratuwa</p>",
      });

      return res.status(200).send(updateSession)``;
    } catch (error) {
      console.log(error, "error!!");
    }
  }),
